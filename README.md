...
vibelign doctor
vibelign anchor
vibelign patch "add progress bar"
vibelign explain
vibelign guard
...
```bash
vibelign init
vibelign checkpoint "first save"
vibelign doctor
...
```bash
vibelign init
vibelign checkpoint "project start"
...
VibeLign는 여러분의 소중한 프로젝트를 안전하게 지켜줍니다.
...
Claude Code, Cursor, OpenCode 같은 AI 도구를 쓸 때 이런 일이 자주 생깁니다. VibeLign는 AI가 안전하게 코드를 고칠 수 있도록 **가이드라인(안전 난간)**을 쳐주고, 언제든 이전 상태로 되돌아갈 수 있는 **세이브 포인트** 기능을 제공합니다.

---

## AI 코딩 작업 순서

VibeLign가 추천하는 안전한 개발 순서:
...
```bash
vibelign init
vibelign checkpoint "첫 번째 저장"
vibelign doctor
vibelign anchor --dry-run
vibelign patch "진행 표시바 추가" --json
vibelign explain --json
vibelign guard --json
```
...
```bash
vibelign init
vibelign checkpoint "프로젝트 시작"

# AI 작업 전
vibelign doctor --strict
vibelign anchor
vibelign patch "원하는 변경사항"
...
VibeLign는 AI 개발에 **구조를 생각하는 안전함**을 더해줍니다 — 그리고 누구나 쉽게 저장하고 되돌릴 수 있게 해줍니다.
