...
from vibeguard.core.doctor_v2 import analyze_project_v2
...
def _rewrite_recommendations(recommendations: List[str]) -> List[str]:
    ...
    for item in recommendations:
        rewritten.append(
            item.replace("`vibeguard anchor`", "`vib anchor --suggest`")
            .replace("vibeguard undo", "vib undo")
            .replace("vibeguard", "vib")
        )
    return rewritten
